...
class GeoPosConv:
    ...
    def set_llh_nmea_degrees(self, latd, lond, h):
        ...
        lad = floor(float(latd) / 100.0)
        lat = float(latd) - lad * 100.0
        lod = floor(float(lond) / 100.0)
        lon = float(lond) - lod * 100.0
    ...
    def conv_llh2xyz(self):
        ...
        PA = 1.0 + 3.0 / 4.0 * pow(Pe, 2) + 45.0 / 64.0 * pow(Pe, 4) + 175.0 / 256.0 * pow(Pe, 6) + 11025.0 \
        ...
        PB = 3.0 / 4.0 * pow(Pe, 2) + 15.0 / 16.0 * pow(Pe, 4) + 525.0 / 512.0 * pow(Pe, 6) + 2205.0 / 2048.0 \
        ...
        PC = 15.0 / 64.0 * pow(Pe, 4) + 105.0 / 256.0 * pow(Pe, 6) + 2205.0 / 4096.0 * pow(Pe, 8) + 10395.0 \
        ...
        PF = 693.0 / 131072.0 * pow(Pe, 10) + 9009.0 / 524288.0 * pow(Pe, 12) + 4099095.0 / 117440512.0 * \
            pow(Pe, 14) + 26801775.0 / 469762048.0 * pow(Pe, 16)

        PG = 3003.0 / 2097152.0 * pow(Pe, 12) + 315315.0 / 58720256.0 * pow(Pe, 14) + 11486475.0 / 939524096.0 \
            * pow(Pe, 16)
        ...
        PS = PB1 * self.m_lat + PB2 * sin(2.0 * self.m_lat) + PB3 * sin(4.0 * self.m_lat) + PB4 \
        ...
        PSo = PB1 * self.m_PLato + PB2 * sin(2.0 * self.m_PLato) + PB3 * sin(4.0 * self.m_PLato) + PB4 \
        ...
        PDL = self.m_lon - self.m_PLo
        Pt = tan(self.m_lat)
        PW = sqrt(1.0 - pow(Pe, 2) * pow(sin(self.m_lat), 2))
        PN = AW / PW
        Pnn = sqrt(pow(Pet, 2) * pow(cos(self.m_lat), 2))

        self.m_x = ((PS - PSo) + (1.0 / 2.0) * PN * pow(cos(self.m_lat), 2.0) * Pt * pow(PDL, 2.0) \
        ...
        self.m_y = (PN * cos(self.m_lat) * PDL - 1.0 / 6.0 * PN * pow(cos(self.m_lat), 3) * (-1 + pow(Pt, 2) \
